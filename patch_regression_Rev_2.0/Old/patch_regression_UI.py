...
def main():
    ...
    tk.Checkbutton(root, text='Delete Existing Patch in BIOS', variable=var_delpatch, onvalue=0, offvalue=1, state='active', font=button_font).grid(row=11, column=0,sticky=tk.W, pady=(20,0))
    tk.Checkbutton(root, text='Copy Stitched BIOS and Patch', variable=var_copy, onvalue=0, offvalue=1, state='active', font=button_font).grid(row=12, column=0, sticky=tk.W)
    tk.Checkbutton(root, text='Flash Bios', variable=var_flash, font=button_font).grid(row=14, column=0, sticky=tk.W)
    tk.Checkbutton(root, text='HT ON', variable=var_hton, font=button_font).grid(row=15, column=0, sticky=tk.W)
    tk.Checkbutton(root, text='HT OFF', variable=var_htoff, font=button_font).grid(row=16, column=0, sticky=tk.W)
