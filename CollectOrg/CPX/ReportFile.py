...
class ReportFile(object):
    ...
    def parseCafeMap(self,filedata):
        ...
        if len(list(self.map_cafe2apic.keys()))==0:
    ...
    def parseInstructions(self,filedata):
        ...
        for linenumber,line in enumerate(filedata):
            ...
            if match2 != None:
                ...
                if instr.thread not in self.random_instrs_by_thread:
    ...
    def write_pickle(self):
        pickleable={}
        for thread,instrs in list(self.random_instrs_by_thread.items()):
            pickleable[thread]=[]
            for i in instrs:
                nexti = copy(i)
        ...
        try:
            ...
            pstr = pickle.dumps(self.__dict__, pickle.HIGHEST_PROTOCOL)
    ...
    def read_pickle(self):
        ...
        self.__dict__ = pickle.loads(pstr)
        
        self.instrs_by_cs_rip = {}
        self.instrs_by_LA = {}
        for instrs in list(self.random_instrs_by_thread.values()):
            previnstr = instrs[0]
            try:
        ...
        for checkpoints in list(self.checkpoints.values()):
            for checkp in list(checkpoints.values()):
