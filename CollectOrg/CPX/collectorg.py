...
# import sys
# sys.path.append(r'C:\pythonsv\cooperlakex\core\hopper')
...
class PostElFile(ResFile):
    def __init__(self, resfile):
        super(PostElFile, self).__init__(resfile)
    def parse(self):
        """Parse the threads and cs:rip for the threads in the file pointed to by filename"""
        import re
        threadslist = []
...
class DotResFile(ResFile):
    def __init__(self, resfile):
        super(DotResFile, self).__init__(resfile)
    def parse(self):
        """Parse the threads and cs:rip for the threads in the file pointed to by filename"""
        import re
        threadslist = []
...
class ResFileCPX(ResFile): #Class to work with CPX system. Carlos
    def __init__(self, resfile):
        super(ResFileCPX, self).__init__(resfile)
    def parse(self):
        """Parse the threads and cs:rip for the threads in the file pointed to by filename"""
...
class Collect:
    ...
    def start(self,resfile, rptfile, apath):
        ...
        if self.resfilename.endswith("eip_dump.json"):
            ...
            self.resfile = ResFileCPX(self.resfilename)
...
def get_latest_folder(path):
    """Find the latest folder created - Carlos"""
