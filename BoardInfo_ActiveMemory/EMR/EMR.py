...
class EMR(Device.Device):
    ...
    def get_unit_info(self):
        return super(EMR, self).get_unit_info_lock_device()

    def _get_pch(self, itp):
        try:
            logger.info("Get pch (EMR)")
            ...
            error_msg = r"_Get_Pch exception on EMR:  {}".format(str(e))
    ...
    def get_activememory(self):
        ...
        import emeraldrapids.mc.emrAddressTranslator as at
        super(EMR, self).get_memory()
        activeMemSize = []
    ...
    def _get_cpu(self, itp):
        try:
            ...
            error_msg = r"_Get_Cpu exception on EMR:  {}".format(str(e))
            logger.error(error_msg)
    ...
    def getMicroCodePatch(self):
        try:
            logger.info("Getting MicroCodePatch (EMR)")
    ...
    def ensure_unlock(self, itp):
        EMR_device = ''
        return super(EMR, self).ensure_unlock(itp, EMR_device)

    def getPlatformInfo(self):
        return super(EMR, self).getPlatformInfo()

    def getDokPlatformInfo(self):
        return super(EMR, self).getDokPlatformInfo()
    ...
    def get_platform_id(self):
        try:
            ...
            return super(EMR, self).get_platform_id(platform_id_cr)
        except Exception as e:
    ...
    def getIfwiInfo(self):
        return super(EMR, self).getIfwiInfo()
    ...
    def _set_cstate(self, cstate, required_value=None):
        try:
            ...
            error_msg = r"_set_cstate exception on EMR:  {}".format(str(e))
            logger.error(error_msg)
            errors_handler.ErrorsHandler.add_error('CPU', error_msg)
