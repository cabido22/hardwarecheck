...
class SKX(SKL):
    ...
    def get_unit_info(self):
        result = super(SKX, self).get_unit_info()
        return result

    def initializeSKL(self):
        super(SKX, self).initializeSKL(device_list_index=0, uncore_index=0)

    def get_platform_id(self):
        return super(SKX, self).get_platform_id()

    def getMicroCodePatch(self):
        return super(SKX, self).getMicroCodePatch()
    ...
    def get_activememory(self):
        ...
        import skylakex.mc.skxAddressTranslator as at
        activeMemSize = []
    ...
    def get_pcie(self):
        try:
            import skylakex.pcie.ltssm_skx as ltssm
    ...
    def get_memoryinfo(self):
        try:
            ...
            import skylakex.mc.skxdimminfo as dimminfo
                ...
                for j in range(0, len(itp.devicelist)):
                    try:
                        if itp.devicelist[j].alias == name:
                            skx = super(SKX, self)._get_cpu(itp,did)
                            cpu_info.append(skx)
                    except Exception as e:
                        logger.debug("SKX get Cpu [" + str(i) + "] exception: " + str(e))
            return cpu_info
        except Exception as e:
            error_msg = r"_Get_Cpu exception on SKX:  {}".format(str(str(e)))
            logger.error(error_msg)
            errors_handler.ErrorsHandler.add_error('CPU', error_msg)
            return UnitInfo("", "", "", "")

    def _get_pch(self, itp):
        super(SKX, self)._get_pch(itp)

    def ensure_unlock(self, itp):
        skx_device = ''
        return super(SKX, self).ensure_unlock(itp, skx_device)

    def getPlatformInfo(self):
        import skylakex.fuse.skxFuseUtility as fu
        try:
            ...
            while unit_index < 2:
                ...
                logger.info("unit[{}]: #cores={} #threads={}".format(unit_index, self.CpuInfo.Cpus[unit_index].num_of_cores,
            ...
            # for i in range(0, unit_index + 1):
                # logger.info("unit[{}]: #cores={} #threads={}".format(i, self.CpuInfo.Cpus[i].num_of_cores,
                                                                     # self.CpuInfo.Cpus[i].num_of_threads))
        except Exception as e:
    ...
    def getDokPlatformInfo(self):
        return super(SKX, self).getDokPlatformInfo()

    def getIfwiInfo(self):
        return super(SKX, self).getIfwiInfo()
